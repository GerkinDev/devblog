...
const rewrapVarLabel = (label: HTMLElement) => {
  ...
  const parentLink: HTMLAnchorElement = findParentMatching<HTMLAnchorElement>(
    label,
    (el) => el.tagName === "A",
  );
  if (parentLink) {
...
const findParentMatching = <T extends Element>(
  el: Element,
  assert: ((el: Element) => el is T) | ((el: Element) => boolean),
) => {
...
const isChild = (obj: Element, parentObj: Element) =>
  !!findParentMatching(obj, (el) => el === parentObj);
...
if (helpContainer) {
    ...
    const setVarInstancesText = (text: string) => {
      ...
      varInstances.forEach((vi) => {
        ...
        if (vi.parentLink) {
          ...
          VAR_MATCH_RE.lastIndex = 0;
          vi.parentLink.href = originalHref.replace(
            VAR_MATCH_RE,
            (fullmatch, varName) => {
              const varDesc = vars.find((v) => v.name === varName);
              return (document.getElementById(varDesc.id) as HTMLInputElement)
                .value;
            },
          );
        }
  ...
  const varsElements = Array
    ...
    .map(rewrapVarLabel)
    .filter((v): v is Exclude<ReturnType<typeof rewrapVarLabel>, undefined> =>
      !!v
    );
  const vars = varsElements
    .filter((value, index, self) =>
      self.findIndex((value2) => value2.name === value.name) === index
    )
    ...
    if (varsContainer) {
      vars.forEach((v) => {
        const targetContainer = v.segments.reduce((acc, segment) => {
          const segmentChild = acc.querySelector<HTMLElement>(
            `:scope > [data-scope="${segment}"]`,
          );
          if (segmentChild) {
            return segmentChild;
          }
